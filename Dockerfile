...
RUN ["/bootstrap/sh", "-c", "rel_date=$(/bootstrap/date +%Y.%m.01) && /bootstrap/wget --timeout=60 -O /bootstrap/archlinux.tar.gz http://lon.mirror.rackspace.com/archlinux/iso/latest/archlinux-bootstrap-${rel_date}-x86_64.tar.gz && /bootstrap/tar --exclude=root.x86_64/etc/resolv.conf --exclude=root.x86_64/usr/share/locale --exclude=root.x86_64/etc/hosts --exclude=root.x86_64/usr/share/terminfo -xvf /bootstrap/archlinux.tar.gz --strip-components=1 -C / && /bin/bash -c 'chmod +x /root/*.sh && /bin/bash /root/install.sh'"]
