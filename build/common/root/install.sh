...
echo -e "export BASE_RELEASE_TAG=${RELEASETAG}\nexport TARGETARCH=${TARGETARCH}" >> '/etc/image-build-info'
...
pacman -S base which awk sed grep gzip supervisor nano vi ldns moreutils bind-tools net-tools dos2unix unzip unrar htop jq ca-certificates ca-certificates-mozilla openbsd-netcat rsync --noconfirm
